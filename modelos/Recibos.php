...
$conectar=parent::conexion();  
  ...
  //*************Clasificar por numero de abonos si es venta o recuperado si el numero de abonos es >0
...
public function get_recibos_emitidos($sucursal){
 ...
 public function get_recibo_por_num($sucursal){
    $conectar= parent::conexion();
    $sql="select*from recibos where sucursal=? and numero_recibo=?;";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1, $sucursal);
    $sql->execute();
    return $resultado= $sql->fetchAll(PDO::FETCH_ASSOC);
...
public function show_datos_recibo($id_paciente,$id_credito,$numero_venta){
    $conectar= parent::conexion();
    $sql="select*from recibos where id_recibo=?;";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1, $id_recibo);
...
public function editar_recibo($a_anteriores,$n_recibo,$n_venta_recibo_ini,$monto,$fecha,$sucursal,$id_paciente,$id_usuario,$telefono_ini,$recibi_rec_ini,$empresa_ini,$texto,$numero,$saldo,$forma_pago,$marca_aro_ini,$modelo_aro_ini,$color_aro_ini,$lente_rec_ini,$ar_rec_ini,$photo_rec_ini,$observaciones_rec_ini,$pr_abono,$servicio_rec_ini){

$conectar=parent::conexion();  

  $sql="update recibos set n_recibo=?,n_venta_recibo_ini=?,monto=?,fecha=?,sucursal=?,id_paciente=?,id_usuario=?,telefono_ini=?,recibi_rec_ini=?,empresa_ini=?,texto=?,a_anteriores=?,numero=?,saldo=?,forma_pago=?,marca_aro_ini=?,modelo_aro_ini=?,color_aro_ini=?,lente_rec_ini=?,ar_rec_ini=?,photo_rec_ini=?,observaciones_rec_ini=?,pr_abono=?,servicio_rec_ini=? where id_recibo=?;";
  $sql=$conectar->prepare($sql);

  $sql->bindValue(1,$_POST["n_recibo"]);
  $sql->bindValue(2,$_POST["n_venta_recibo_ini"]);
  $sql->bindValue(3,$_POST["monto"]);
  $sql->bindValue(4,$_POST["fecha"]);
  $sql->bindValue(5,$_POST["sucursal"]);
  $sql->bindValue(6,$_POST["id_paciente"]);
  $sql->bindValue(7,$_POST["id_usuario"]);
  $sql->bindValue(8,$_POST["telefono_ini"]);
  $sql->bindValue(9,$_POST["recibi_rec_ini"]);
  $sql->bindValue(10,$_POST["empresa_ini"]);
  $sql->bindValue(11,$_POST["texto"]);
  $sql->bindValue(12,$_POST["a_anteriores"]);
  $sql->bindValue(13,$_POST["numero"]);
  $sql->bindValue(14,$_POST["saldo"]);
  $sql->bindValue(15,$_POST["forma_pago"]);
  $sql->bindValue(16,$_POST["marca_aro_ini"]);
  $sql->bindValue(17,$_POST["modelo_aro_ini"]);
  $sql->bindValue(18,$_POST["color_aro_ini"]);
  $sql->bindValue(19,$_POST["lente_rec_ini"]);
  $sql->bindValue(20,$_POST["ar_rec_ini"]);
  $sql->bindValue(21,$_POST["photo_rec_ini"]);
  $sql->bindValue(22,$_POST["observaciones_rec_ini"]);
  $sql->bindValue(23,$_POST["pr_abono"]);
  $sql->bindValue(24,$_POST["servicio_rec_ini"]);
  
  $sql->execute();

  ///////////////REGISTRA ABONOS
  $sql2="update recibos set abono_act=?,forma_pago=?,fecha=?,id_paciente=?,id_usuario=?,n_recibo=?,numero_venta=?,sucursal=? where id_recibo=?;";
  $sql2=$conectar->prepare($sql2);
  $sql2->bindValue(1,$_POST["numero"]);
  $sql2->bindValue(2,$_POST["forma_pago"]);
  $sql2->bindValue(3,$_POST["fecha"]);
  $sql2->bindValue(4,$_POST["id_paciente"]);
  $sql2->bindValue(5,$_POST["id_usuario"]);
  $sql2->bindValue(6,$_POST["n_recibo"]);
  $sql2->bindValue(7,$_POST["n_venta_recibo_ini"]);
  $sql2->bindValue(8,$_POST["sucursal"]);
  $sql2->execute();
  ...
  $resultados = $sql3->fetchAll(PDO::FETCH_ASSOC);
    ...
    $forma_venta =$row["tipo_credito"];
      ...
      if(is_array($resultados)==true and count($resultados)>0) {                     
      //actualiza el stock en la tabla producto
        $sql12 = "update creditos set saldo=? where numero_venta=? and id_paciente=? where id_recibo=?";
        $sql12 = $conectar->prepare($sql12);
        $sql12->bindValue(1,$_POST["saldo_act"]);
        $sql12->bindValue(2,$_POST["n_venta_recibo_ini"]);
        $sql12->bindValue(3,$_POST["id_paciente"]);
        //$sql12->bindValue(3,$sucursal);
        $sql12->execute();               
    }//Fin del if
...
/////////////////////EXTRAER EL TIPO DE PAGO
 ...
 $sql17="update recibos set fecha_ingreso=?,n_recibo=?,n_venta=?,n_factura=?,paciente=?,vendedor=?,total_factura=?,forma_cobro=?,monto_cobrado=?,saldo_credito=?,tipo_venta=?,tipo_pago=?,id_usuario=?,abono_anterior=?,abonos_realizados=?,id_paciente=?,sucursal_venta=?,sucursal_cobro=?,tipo_ingreso";
  $sql17=$conectar->prepare($sql17);
  $sql17->bindValue(1,$fecha);
  $sql17->bindValue(2,$n_recibo);
  $sql17->bindValue(3,$n_venta_recibo_ini);
